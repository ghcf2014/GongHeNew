		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。（*多张以上请一次性选取，否则系统默认为单张上传。）</div>';
		            html += '				<div class="btns">';
				    ...
				    html += '							<img class="upload_image" src="/Public/Static/zyUpload/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="photo[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">选择文件</div>';
		            html += '					<div class="upload_btn">开始上传</div>';
		            html += '				</div>';
		            html += '			</div>';
				    ...
				    html += '					<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" title="点击添加文件" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
				    html += '						<div class="uploadImg" style="width:'+imgWidth+'px">';
				    html += '							<img class="upload_image" src="zyUpload/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$("#status_info").html("选中"+num+"张文件，共"+size+"。（*多张以上请一次性选取，否则系统默认为单张上传。）");
			};
